...
- **Destruction replacements distinguish destruction from other exits.** The
  ...
  choice. A plain `Destroy` always permits regeneration; Wrath of God and
  other clauses whose later text modifies that destruction wrap it in an
  effect-local `CannotRegenerate` rule, while "this turn" clauses use the same
  rule with an ordinary resolved duration. The local rule reconstructs from
  its declarative effect path, so checkpoint format 11 remains unchanged.
  Clergy of the Holy Nimbus, Knight of the Holy Nimbus, Mossbridge Troll,
  Crackling Emergence, Harmonious Emergence, and Pyramids use these pieces
...
- **A split card or Room is worth both halves anywhere but the stack.** Its
  ...
  Walk-In Closet // Forgotten Cellar charged three rather than eight, and a
  mill that totalled what it buried counted the same way. Cards read in a
  zone now combine their halves, doors included; a predicate that names one
