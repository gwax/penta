...
// Audit: unsupported — Needs level-granted static anthem evaluation and opponent spell taxation
// during your turn.
...
pub(in crate::card::sets) static SHAMBLING_GHAST: CardRecord = CardRecord::new(
    "Shambling Ghast",
...
// Audit: unsupported — Needs each opponent's total life lost this turn, rather than net life
// change.
...
// Audit: unsupported — Attack history cannot sum the power of creatures as they were declared
// this combat. A resolution-time query of attacking creatures changes after removal and pump
// effects and cannot implement the intervening-if pack-tactics check.
pub(in crate::card::sets) static BATTLE_CRY_GOBLIN: CardRecord = CardRecord::new(
    "Battle Cry Goblin",
...
// Audit: unsupported — The turn-history vocabulary does not count creatures of a specified type
// entering under a player's control. A live EnteredThisTurn query would lose Goblins that have
// since left the battlefield.
pub(in crate::card::sets) static HOBGOBLIN_BANDIT_LORD: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static PLUNDERING_BARBARIAN: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static UNEXPECTED_WINDFALL: CardRecord = CardRecord::new(
    "Unexpected Windfall",
...
pub(in crate::card::sets) static CIRCLE_OF_DREAMS_DRUID: CardRecord = CardRecord::new(
...
// AFR 180 — Druid Class
// Audit: unsupported — Needs level-granted additional-land permission and durable animation
// with a granted characteristic-defining ability.
...
pub(in crate::card::sets) static THE_TARRASQUE: CardRecord = CardRecord::new(
    "The Tarrasque",
    "8a26fa15-d81f-4152-ae33-e91aa276b3fc",
    "Filip Burburan",
    CardRules::new_creature(mana_cost!("{6}{G}{G}{G}"), &["Dinosaur"], 10, 10)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            AbilityDef::static_ability(
                "The Tarrasque has haste and ward {10} as long as it was \
                 cast.",
                EffectDef::IfCondition {
                    condition: &TriggerConditionDef::SourceWasCast,
            ...
            AbilityDef::triggered_with_targets(
                "Whenever The Tarrasque attacks, it fights target creature \
                ...
                EffectDef::Fight {
                    first: crate::card::ObjectRefDef::Source,
                    second: crate::card::ObjectRefDef::Target(TargetIndex::PRIMARY),
                    excess: None,
                },
            ),
        ]),
...
// AFR 217 — Bard Class
// Audit: unsupported — Needs level-granted static colored-mana cost modifiers; the existing
// colored-symbol reduction is read only from intrinsic clauses.
...
// Audit: unsupported — Needs equip-only activation cost reduction and mandatory blocking of a
// particular attacker.
...
// AFR 228 — Monk Class
// Audit: unsupported — Needs card-bound exile casting permission conditional on another spell
// cast this turn.
...
// AFR 230 — Rogue Class
// Audit: unsupported — Needs source-linked face-down exile with persistent look permission and
// level-gated play permission.
...
// Audit: unsupported — Needs mana restricted specifically to Class-level bar activations,
// alongside instant or sorcery casting.
...
pub(in crate::card::sets) static HIVE_OF_THE_EYE_TYRANT: CardRecord = CardRecord::new(
    ...
    CardRules::new_land(&[]).with_abilities(&[
        AbilityDef::as_enters(
            ...
            ReplacementEffectDef::Conditional {
                ...
                if_true: &[ReplacementEffectDef::ModifyBattlefieldEntry(
        ...
        AbilityDef::activated(
            "{3}{B}: Until end of turn, this land becomes a 3/3 black \
             ...
             attacks, exile target card from defending player's \
            ...
            EffectDef::Apply {
                ...
                effect: AppliedEffectDef::Composite(&[
                    ...
                    AppliedEffectDef::set_creature_types(CreatureTypeSetDef::named(&["Beholder"])),
                    AppliedEffectDef::add_ability(&abilities::menace()),
                    AppliedEffectDef::add_ability(&AbilityDef::triggered_with_targets(
                        "Whenever this creature attacks, exile target card from \
...
pub(in crate::card::sets) static OLD_GNAWBONE: CardRecord = CardRecord::new(
    "Old Gnawbone",
    "16ead969-8ba4-4587-a68d-47730943605e",
    "Randy Vargas",
    CardRules::new_creature(mana_cost!("{5}{G}{G}"), &["Dragon"], 7, 7)
        .with_abilities(&[
            abilities::flying(),
            AbilityDef::triggered(
                "Whenever a creature you control deals combat damage to a \
                 player, create that many Treasure tokens.",
...
pub(in crate::card::sets) static OSWALD_FIDDLEBENDER: CardRecord = CardRecord::new(
    "Oswald Fiddlebender",
    "853c9db7-504e-4dfb-8067-abd2a36f6a1a",
    "Phil Stone",
    CardRules::new_creature(mana_cost!("{1}{W}"), &["Gnome", "Artificer"], 2, 2)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[AbilityDef::activated(
            "Magical Tinkering — {W}, {T}, Sacrifice an artifact: Search \
             your library for an artifact card with mana value equal to \
             1 plus the sacrificed artifact's mana value, put it onto \
...
// Audit: unsupported — The repeating d20 roll with an optional reroll has no declarative dice
// procedure.
pub(in crate::card::sets) static DELINA_WILD_MAGE: CardRecord = CardRecord::new(
    "Delina, Wild Mage",
...
// AFR 322 — Xorn
// Audit: unsupported — Its replacement needs to recognize Treasure-token creation and add one
// to that event.
pub(in crate::card::sets) static XORN: CardRecord = CardRecord::new(
    "Xorn",
...
pub(in crate::card::sets) static DEN_OF_THE_BUGBEAR: CardRecord = CardRecord::new(
    "Den of the Bugbear",
    ...
    CardRules::new_land(&[]).with_abilities(&[
        ...
        AbilityDef::activated(
            "{3}{R}: Until end of turn, this land becomes a 3/2 red \
             Goblin creature with \"Whenever this creature attacks, \
             create a 1/1 red Goblin creature token that's tapped and \
             attacking.\" It's still a land.",
            ...
            EffectDef::Apply {
                ...
                effect: AppliedEffectDef::Composite(&[
                    ...
                    AppliedEffectDef::set_creature_types(CreatureTypeSetDef::named(&["Goblin"])),
                    AppliedEffectDef::set_colors(ColorSet::from_colors(&[ManaColor::Red])),
                    AppliedEffectDef::add_ability(&AbilityDef::triggered(
                        "Whenever this creature attacks, create a 1/1 red Goblin \
                         creature token that is tapped and attacking.",
                        TriggerEventDef::attacks(ObjectPredicateDef::Source),
                        EffectDef::CreateToken(
                            crate::card::CreateTokenDef::new(crate::card::TokenDef::Literal(
...
pub(in crate::card::sets) static TREASURE_VAULT: CardRecord = CardRecord::new(
    "Treasure Vault",
...
pub(in crate::card::sets) static LOYAL_WARHOUND: CardRecord = CardRecord::new(
    "Loyal Warhound",
    ...
    CardRules::new_creature(mana_cost!("{1}{W}"), &["Dog"], 3, 1).with_abilities(&[
        ...
        AbilityDef::triggered_if(
            "When this creature enters, if an opponent controls more \
             lands than you, search your library for a basic Plains \
...
// Audit: unsupported — Its draw replacement needs a face-down exile pile tied to the later
// life-payment action.
pub(in crate::card::sets) static ASMODEUS_THE_ARCHFIEND: CardRecord = CardRecord::new(
