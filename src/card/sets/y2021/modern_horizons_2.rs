...
pub(in crate::card::sets) static SANCTIFIER_EN_VEC: CardRecord = CardRecord::new(
    "Sanctifier en-Vec",
...
// MH2 39 — Dress Down
pub(in crate::card::sets) static DRESS_DOWN: CardRecord = CardRecord::new(
    "Dress Down",
...
// MH2 61 — Scuttletide
pub(in crate::card::sets) static SCUTTLETIDE: CardRecord = CardRecord::new(
    "Scuttletide",
    ...
    CardRules::new_enchantment(mana_cost!("{1}{U}")).with_abilities(&[
        ...
        AbilityDef::static_ability(
            "Delirium — Crabs you control get +1/+1 as long as there are \
...
// MH2 66 — Step Through
pub(in crate::card::sets) static STEP_THROUGH: CardRecord = CardRecord::new(
    "Step Through",
            ...
            [
                ...
                abilities::enters_trigger_with_targets(
                    "When this creature enters, choose up to one target creature \
                     spell or planeswalker spell. Its owner puts it on their \
...
// MH2 68 — Suspend
pub(in crate::card::sets) static SUSPEND: CardRecord = CardRecord::new(
    "Suspend",
    "40af215c-d3f7-42f0-85cd-77a3fcd919ba",
    "Lake Hurwitz",
    CardRules::new_instant(mana_cost!("{U}")).with_abilities(&[AbilityDef::spell_with_targets(
        "Exile target creature and put two time counters on it. If \
         it doesn't have suspend, it gains suspend. (At the \
         beginning of its owner's upkeep, they remove a time \
         counter. When the last is removed, they may play it without \
        ...
        EffectDef::BindObjects(BindObjectsDef {
            ...
            binding: Binding!("suspend_target"),
            then: &EffectDef::Sequence(&[
                EffectDef::move_to_zone(
                    EffectRecipientDef::objects(ObjectSetDef::Binding(Binding!("suspend_target"))),
                    ZoneKind::Exile,
                    ZonePlacement::Top,
                ),
                EffectDef::BindObjects(BindObjectsDef {
                    source: ObjectCollectionSourceDef::ObjectSet(
                        ObjectSetDef::ZoneChangeSuccessorsOfBinding(Binding!("suspend_target")),
                    ),
                    binding: Binding!("suspend_exiled"),
                    then: &EffectDef::ForEachInBinding {
                        objects: Binding!("suspend_exiled"),
                        binding: Binding!("suspend_card"),
                        effect: &EffectDef::Sequence(&[
                            EffectDef::AddCounters {
                                object: EffectRecipientDef::object(ObjectRefDef::Binding(
                                    Binding!("suspend_card"),
                                )),
                                ...
                                condition: &TriggerConditionDef::Not(
                                    &TriggerConditionDef::BoundObjectMatches {
                                        ...
                                        object: ObjectPredicateDef::HasAbility(
                                            AbilityPredicateDef::Is(AbilityKindDef::Suspend),
                                        ),
                                    },
                                ),
                                then: &EffectDef::Apply {
                                    recipient: EffectRecipientDef::object(ObjectRefDef::Binding(
                                        Binding!("suspend_card"),
                                    )),
                                    effect: AppliedEffectDef::add_ability(&abilities::suspend(
                                        "Suspend",
                                        &SuspendAbilityDef::granted(),
                                    )),
            ...
            [
                ...
                abilities::enters_trigger_with_targets(
                    "When this creature enters, target opponent reveals their \
                     hand. You choose a nonland card from it. That player \
                     discards that card.",
                    &[AbilityTargetDef::exactly_one(
                        AbilityTargetPredicate::Player(PlayerRelation::Opponent),
                    )],
...
// MH2 88 — Hell Mongrel
// Audit: unsupported — Madness needs a discard-to-exile replacement followed by a linked
// triggered cast-or-graveyard choice. No existing alternative-cast procedure implements that
// timing and zone-change sequence.
pub(in crate::card::sets) static HELL_MONGREL: CardRecord = CardRecord::new(
    "Hell Mongrel",
...
// MH2 96 — Persist
pub(in crate::card::sets) static PERSIST: CardRecord = CardRecord::new(
    "Persist",
    "90f390c3-af1c-424f-9721-e26e9321e5a3",
    "Milivoj Ćeran",
    CardRules::new_sorcery(mana_cost!("{1}{B}")).with_ability(AbilityDef::spell_with_targets(
        "Return target nonlegendary creature card from your \
         graveyard to the battlefield with a -1/-1 counter on it.",
        &[AbilityTargetDef::exactly_one(
            AbilityTargetPredicate::Object {
                object: ObjectPredicateDef::All(&[
                    ObjectPredicateDef::HasType(CardType::Creature),
...
pub(in crate::card::sets) static TOURACH_DREAD_CANTOR: CardRecord = CardRecord::new(
    "Tourach, Dread Cantor",
    "f3526751-0101-4d91-a496-c53cd92326e0",
    "Greg Staples",
    CardRules::new_creature(mana_cost!("{1}{B}"), &["Human", "Cleric"], 2, 1)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            abilities::kicker(&[CostDef::Mana(mana_cost!("{B}{B}"))]),
            abilities::protection_from_color(ManaColor::White),
            AbilityDef::triggered(
                "Whenever an opponent discards a card, put a +1/+1 counter \
                 on Tourach.",
...
pub(in crate::card::sets) static UNMARKED_GRAVE: CardRecord = CardRecord::new(
    "Unmarked Grave",
...
// triggered cast-or-graveyard choice. No existing alternative-cast procedure implements that
// timing and zone-change sequence.
pub(in crate::card::sets) static BLAZING_ROOTWALLA: CardRecord = CardRecord::new(
    "Blazing Rootwalla",
...
pub(in crate::card::sets) static DRAGON_S_RAGE_CHANNELER: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{R}"), &["Human", "Shaman"], 1, 1).with_abilities(&[
        AbilityDef::triggered(
            "Whenever you cast a noncreature spell, surveil 1. (Look at \
             the top card of your library. You may put that card into \
             your graveyard.)",
...
// Audit: unsupported — ExilePlayDurationDef only offers permissions starting immediately. It
// cannot defer the play window until the next turn and disallow playing the exiled card during
// the current turn.
pub(in crate::card::sets) static GALVANIC_RELAY: CardRecord = CardRecord::new(
    "Galvanic Relay",
...
// arbitrary triggered abilities of Shamans and other Wizards, including prowess itself.
pub(in crate::card::sets) static HARMONIC_PRODIGY: CardRecord = CardRecord::new(
    "Harmonic Prodigy",
...
// triggered cast-or-graveyard choice. No existing alternative-cast procedure implements that
// timing and zone-change sequence.
pub(in crate::card::sets) static REVOLUTIONIST: CardRecord = CardRecord::new(
    "Revolutionist",
...
pub(in crate::card::sets) static STRIKE_IT_RICH: CardRecord = CardRecord::new(
    "Strike It Rich",
...
pub(in crate::card::sets) static TAVERN_SCOUNDREL: CardRecord = CardRecord::new(
    "Tavern Scoundrel",
    ...
    CardRules::new_creature(mana_cost!("{1}{R}"), &["Human", "Rogue"], 1, 3).with_abilities(&[
        AbilityDef::triggered(
            "Whenever you win a coin flip, create two Treasure tokens. \
             (They're artifacts with \"{T}, Sacrifice this token: Add \
        ...
        AbilityDef::activated(
            ...
            &[
                CostDef::Mana(mana_cost!("{1}")),
                CostDef::TapSource,
                CostDef::sacrifice_permanent(ObjectPredicateDef::Not(&ObjectPredicateDef::Source)),
            ],
            EffectDef::FlipCoin {
...
pub(in crate::card::sets) static UNHOLY_HEAT: CardRecord = CardRecord::new(
    ...
    CardRules::new_instant(mana_cost!("{R}")).with_ability(AbilityDef::spell_with_targets(
        "Unholy Heat deals 2 damage to target creature or \
         planeswalker.\nDelirium — Unholy Heat deals 6 damage \
         instead if there are four or more card types among cards in \
         your graveyard.",
        &[AbilityTargetDef::exactly_one(
            AbilityTargetPredicate::Object {
                object: ObjectPredicateDef::AnyOf(&[
...
// Audit: unsupported — Needs a reveal-until whose unmatched cards go somewhere other than the
// graveyard. MillUntilDef mills everything it passes over, where this puts the rest on the
...
// Audit: unsupported — Token replacement only multiplies the prospective token batch. It cannot
// append one independently declared Squirrel token for each token in the replaced batch.
pub(in crate::card::sets) static CHATTERFANG_SQUIRREL_GENERAL: CardRecord = CardRecord::new(
            ...
            [
                ...
                abilities::enters_trigger_with_targets(
                    "When this creature enters, up to one target player puts all \
                     the cards from their graveyard on the bottom of their \
...
pub(in crate::card::sets) static GRIST_THE_HUNGER_TIDE: CardRecord = CardRecord::new(
    ...
    CardRules::new_planeswalker(mana_cost!("{1}{B}{G}"), &["Grist"], 3)
        ...
        .with_abilities(&[
            AbilityDef::static_ability(
                "As long as Grist isn't on the battlefield, it's a 1/1 \
...
// triggered cast-or-graveyard choice. No existing alternative-cast procedure implements that
// timing and zone-change sequence.
pub(in crate::card::sets) static TERMINAL_AGONY: CardRecord = CardRecord::new(
    "Terminal Agony",
        ...
        .with_abilities(&[
            ...
            AbilityDef::static_ability(
                "Equipped creature gets +5/+5 and has first strike, trample, \
                 indestructible, haste, and \"Whenever this creature deals \
                 combat damage to a creature, exile that creature.\"",
                EffectDef::StaticApply {
...
pub(in crate::card::sets) static LIQUIMETAL_TORQUE: CardRecord = CardRecord::new(
    "Liquimetal Torque",
    ...
    CardRules::new_artifact(mana_cost!("{2}")).with_abilities(&[
        ...
        AbilityDef::activated_with_targets(
            "{T}: Target nonland permanent becomes an artifact in \
...
pub(in crate::card::sets) static ORNITHOPTER_OF_PARADISE: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static GOLDMIRE_BRIDGE: CardRecord = CardRecord::new(
    "Goldmire Bridge",
...
pub(in crate::card::sets) static MISTVAULT_BRIDGE: CardRecord = CardRecord::new(
    "Mistvault Bridge",
...
pub(in crate::card::sets) static RAZORTIDE_BRIDGE: CardRecord = CardRecord::new(
    "Razortide Bridge",
...
pub(in crate::card::sets) static ESPER_SENTINEL: CardRecord = CardRecord::new(
    "Esper Sentinel",
    "676758ee-dac8-4c97-8a62-fff25bcbb6df",
    "Eric Deschamps",
    CardRules::new_artifact_creature(mana_cost!("{W}"), &["Human", "Soldier"], 1, 1)
        .with_abilities(&[AbilityDef::triggered(
            "Whenever an opponent casts their first noncreature spell \
             each turn, draw a card unless that player pays {X}, where X \
            ...
            EffectDef::PayOr(
                PayOrDef::unless(
                    &[CostDef::GenericMana(ValueDef::SourcePower)],
...
// Audit: unsupported — Protection predicates cannot consume a stored chosen card type, so a
// static chosen-card-type protection grant to the player and creatures cannot be declared.
pub(in crate::card::sets) static SERRA_S_EMISSARY: CardRecord = CardRecord::new(
    "Serra's Emissary",
...
pub(in crate::card::sets) static THOUGHT_MONITOR: CardRecord = CardRecord::new(
    "Thought Monitor",
...
pub(in crate::card::sets) static KITCHEN_IMP: CardRecord = CardRecord::new(
    "Kitchen Imp",
...
pub(in crate::card::sets) static TIMELESS_WITNESS: CardRecord = CardRecord::new(
    "Timeless Witness",
    ...
    CardRules::new_creature(mana_cost!("{2}{G}{G}"), &["Human", "Shaman"], 2, 1).with_abilities(&[
        ...
        abilities::eternalize!(
            "Eternalize {5}{G}{G} ({5}{G}{G}, Exile this card from your \
             graveyard: Create a token that's a copy of it, except it's \
             a 4/4 black Zombie Human Shaman with no mana cost. \
            ...
            abilities::saga_chapter(
                ...
                EffectDef::Apply {
                    ...
                    effect: AppliedEffectDef::add_ability(&AbilityDef::activated(
                        "{2}, {T}: Create a 0/0 colorless Construct artifact \
...
pub(in crate::card::sets) static PROFANE_TUTOR: CardRecord = CardRecord::new(
    "Profane Tutor",
...
pub(in crate::card::sets) static CHATTERSTORM: CardRecord = CardRecord::new(
    "Chatterstorm",
...
pub(in crate::card::sets) static DAUTHI_VOIDWALKER: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{B}{B}"), &["Dauthi", "Rogue"], 3, 2).with_abilities(&[
        abilities::shadow(),
        AbilityDef::replacement_for(
        ...
        AbilityDef::activated(
            "{T}, Sacrifice this creature: Choose an exiled card an \
             opponent owns with a void counter on it. You may play it \
...
// triggered cast-or-graveyard choice. No existing alternative-cast procedure implements that
// timing and zone-change sequence.
pub(in crate::card::sets) static NECROGOYF: CardRecord = CardRecord::new(
    "Necrogoyf",
...
pub(in crate::card::sets) static SANCTUM_WEAVER: CardRecord = CardRecord::new(
    "Sanctum Weaver",
...
pub(in crate::card::sets) static DIAMOND_LION: CardRecord = CardRecord::new(
    "Diamond Lion",
