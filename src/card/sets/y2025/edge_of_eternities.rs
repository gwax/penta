...
// Audit: unsupported — Needs exile-until-source-leaves with immediate return when the duration ends (CR 610.3); an ordinary leaves trigger returns through the stack too late. Warp is supported.
...
// Audit: unsupported — Needs the source spell's mana-spent amount retained for target selection after the source leaves.
...
// Audit: unsupported — Needs a counter-placement replacement that doubles every counter kind on matching controlled creatures, Spacecraft, and Planets, including entry counters. Warp is supported.
