...
pub(in crate::card::sets) static HAAZDA_SNARE_SQUAD: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{2}{W}"), &["Human", "Soldier"], 1, 4).with_ability(
        AbilityDef::triggered_with_targets(
            "Whenever this creature attacks, you may pay {W}. If you do, \
...
pub(in crate::card::sets) static SCION_OF_VITU_GHAZI: CardRecord = CardRecord::new(
    ...
    "Willian Murai",
    CardRules::new_creature(mana_cost!("{3}{W}{W}"), &["Elemental"], 4, 4).with_abilities(&[
        abilities::vigilance(),
...
pub(in crate::card::sets) static SUNSPIRE_GATEKEEPERS: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{3}{W}"), &["Human", "Soldier"], 2, 4).with_ability(
        AbilityDef::triggered_if(
            "When this creature enters, if you control two or more \
             Gates, create a 2/2 white Knight creature token with \
             vigilance.",
            TriggerEventDef::zone_changed(
                ObjectPredicateDef::Source,
                None,
...
pub(in crate::card::sets) static AETHERLING: CardRecord = CardRecord::new(
    ...
    "Tyler Jacobson",
    CardRules::new_creature(mana_cost!("{4}{U}{U}"), &["Shapeshifter"], 4, 5).with_abilities(&[
        AbilityDef::activated(
            "{U}: Exile this creature. Return it to the battlefield \
...
// Audit: unsupported — Independent tap-or-untap resolution choices are available, but cipher
// still needs an encoded-card link, its combat-damage trigger, and free-copy casting
// permission.
...
// Audit: unsupported — Needs duration-scoped color-word text changes and cipher's encoded-card
// link, combat-damage trigger, and free-copy casting permission.
...
pub(in crate::card::sets) static UNCOVERED_CLUES: CardRecord = CardRecord::new(
    ...
    CardRules::new_sorcery(mana_cost!("{2}{U}")).with_ability(AbilityDef::spell(
        "Look at the top four cards of your library. You may reveal \
         up to two instant and/or sorcery cards from among them and \
         put the revealed cards into your hand. Put the rest on the \
        ...
        EffectDef::ChooseCardsFromCollection(crate::card::ChooseCardsFromCollectionDef {
            ...
            minimum: 0,
            maximum: 2,
            chosen: Binding!("uncovered_clues_chosen"),
            remainder: Binding!("uncovered_clues_remainder"),
            then: &EffectDef::Sequence(&[
                EffectDef::MoveObjects(MoveObjectsDef {
                    input: ObjectSetDef::Binding(Binding!("uncovered_clues_chosen")),
                ...
                EffectDef::ChooseObjectOrder(crate::card::ChooseObjectOrderDef {
                    actor: PlayerRefDef::EffectController,
                    input: ObjectSetDef::Binding(Binding!("uncovered_clues_remainder")),
...
// Audit: unsupported — Hand-size draw adjustments and continued replaced draws exist
// separately, but no replacement-with-effect can both draw an additional card and make its
...
pub(in crate::card::sets) static HIRED_TORTURER: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{2}{B}"), &["Human", "Rogue"], 2, 3).with_ability(
        AbilityDef::activated_with_targets(
            "{3}{B}, {T}: Target opponent loses 2 life, then reveals a \
            ...
            AbilityDef::triggered(
                ...
                TriggerEventDef::AnyOf(&[
                    ...
                    TriggerEventDef::BlocksOrBecomesBlockedBy {
                        creature: ObjectPredicateDef::All(&[
...
pub(in crate::card::sets) static UBUL_SAR_GATEKEEPERS: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{3}{B}"), &["Zombie", "Soldier"], 2, 4).with_ability(
        AbilityDef::triggered_if_with_targets(
            "When this creature enters, if you control two or more \
             Gates, target creature an opponent controls gets -2/-2 \
             until end of turn.",
            TriggerEventDef::zone_changed(
                ObjectPredicateDef::Source,
                None,
...
// Audit: unsupported — Cascade supplies a free-cast offer and random-bottom cleanup for its own
// mana-value search, but no reusable reveal-until procedure matches card types against the
// triggering spell and tracks Possibility Storm's exiled pile.
...
pub(in crate::card::sets) static PUNISH_THE_ENEMY: CardRecord = CardRecord::new(
    ...
    "Slawomir Maniak",
    CardRules::new_instant(mana_cost!("{4}{R}")).with_ability(AbilityDef::spell_with_targets(
        "Punish the Enemy deals 3 damage to target player or \
...
pub(in crate::card::sets) static PYREWILD_SHAMAN: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{2}{R}"), &["Goblin", "Shaman"], 3, 1).with_abilities(&[
        abilities::bloodrush!(
            ...
            "Bloodrush — {1}{R}, Discard this card: Target attacking \
             creature gets +3/+1 until end of turn.",
            &[AbilityTargetDef::exactly_one_permanent(
                ObjectPredicateDef::All(&[
                    ObjectPredicateDef::HasType(CardType::Creature),
        ...
        AbilityDef::triggered(
            "Whenever one or more creatures you control deal combat \
             damage to a player, if this card is in your graveyard, you \
             may pay {3}. If you do, return this card to your hand.",
...
pub(in crate::card::sets) static SMELT_WARD_GATEKEEPERS: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{3}{R}"), &["Human", "Warrior"], 2, 4).with_ability(
        AbilityDef::triggered_if_with_targets(
            "When this creature enters, if you control two or more \
             Gates, gain control of target creature an opponent controls \
...
// Audit: unsupported — Evolve is available, but its counter placement has no distinct event
// from any other +1/+1 counter placement to drive the counter sweep.
...
pub(in crate::card::sets) static BLAST_OF_GENIUS: CardRecord = CardRecord::new(
    ...
    "Terese Nielsen",
    CardRules::new_sorcery(mana_cost!("{4}{U}{R}")).with_ability(AbilityDef::spell_with_targets(
        "Choose any target. Draw three cards, then discard a card. \
         This spell deals damage equal to the discarded card's mana \
         value to that permanent or player.",
        &[AbilityTargetDef::exactly_one(
...
// Audit: unsupported — Needs a damage event that groups all damage dealt by one instant or
// sorcery before creating the two tokens.
...
pub(in crate::card::sets) static BLOOD_BARON_OF_VIZKOPA: CardRecord = CardRecord::new(
    ...
    "Anthony Palumbo",
    CardRules::new_creature(mana_cost!("{3}{W}{B}"), &["Vampire"], 4, 4).with_abilities(&[
        abilities::lifelink(),
        abilities::protection_from_color(ManaColor::White),
        abilities::protection_from_color(ManaColor::Black),
        AbilityDef::static_ability(
            "As long as you have 30 or more life and an opponent has 10 \
             or less life, this creature gets +6/+6 and has flying.",
...
// Audit: unsupported — Needs beginning-of-combat targets that impose positive and negative
// attack-or-block requirements for that combat.
...
pub(in crate::card::sets) static BRED_FOR_THE_HUNT: CardRecord = CardRecord::new(
    ...
    "Karl Kopinski",
    CardRules::new_enchantment(mana_cost!("{1}{G}{U}")).with_ability(AbilityDef::triggered(
...
pub(in crate::card::sets) static BRONZEBEAK_MOA: CardRecord = CardRecord::new(
    ...
    "James Ryman",
    CardRules::new_creature(mana_cost!("{2}{G}{W}"), &["Bird"], 2, 2).with_ability(AbilityDef::triggered(
...
// Audit: unsupported — Regeneration is available, but the global blocking event exposes the
// other creature rather than the blocker whose controller must lose life.
...
// Audit: unsupported — Stored card-name rules and their cast restriction and discount consumers
// exist, but the entry choice cannot be restricted to noncreature, nonland names.
...
pub(in crate::card::sets) static DEADBRIDGE_CHANT: CardRecord = CardRecord::new(
    ...
    "Zoltan Boros",
    CardRules::new_enchantment(mana_cost!("{4}{B}{G}")).with_abilities(&[
        abilities::enters_trigger(
        ...
        AbilityDef::triggered(
            "At the beginning of your upkeep, choose a card at random in \
             your graveyard. If it's a creature card, put it onto the \
...
pub(in crate::card::sets) static DRAGONSHIFT: CardRecord = CardRecord::new(
    ...
    CardRules::new_instant(mana_cost!("{1}{U}{R}")).with_abilities(&[
        AbilityDef::spell_with_targets(
            "Until end of turn, target creature you control becomes a \
             blue and red Dragon with base power and toughness 4/4, \
        ...
        abilities::overload(
            ...
            "Until end of turn, each creature you control becomes a blue \
             and red Dragon with base power and toughness 4/4, loses all \
             abilities, and gains flying.",
            EffectDef::Apply {
                recipient: EffectRecipientDef::matching_objects(
                    ObjectPredicateDef::HasType(CardType::Creature),
                    &[ZoneKind::Battlefield],
...
pub(in crate::card::sets) static DROWN_IN_FILTH: CardRecord = CardRecord::new(
    ...
    CardRules::new_sorcery(mana_cost!("{B}{G}")).with_ability(AbilityDef::spell_with_targets(
        "Choose target creature. Mill four cards, then that creature \
         gets -1/-1 until end of turn for each land card in your \
         graveyard.",
        &[AbilityTargetDef::exactly_one_permanent(
            ObjectPredicateDef::HasType(CardType::Creature),
        )],
    ...
    CardRules::new_creature(mana_cost!("{2}{U}{R}"), &["Weird"], 1, 5).with_abilities(&[
        ...
        AbilityDef::triggered(
            "Whenever you cast an instant or sorcery spell, you may \
             switch this creature's power and toughness until end of \
             turn.",
...
// Audit: unsupported — Needs a uniformly random legal target choice when the activated ability
// resolves.
...
// Audit: unsupported — Needs removing a +1/+1 counter from a chosen creature, rather than from
// the ability source, as an activation cost.
...
// Audit: unsupported — The static prohibitions and ReturnSourceToHand cost exist, but paying
// that cost loses the Aura's attachment before the resolving ability can put counters on its
// former host.
...
pub(in crate::card::sets) static LAVINIA_OF_THE_TENTH: CardRecord = CardRecord::new(
    ...
    "Willian Murai",
    CardRules::new_creature(mana_cost!("{3}{W}{U}"), &["Human", "Soldier"], 4, 4)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            AbilityDef::keyword(
                "Protection from red",
            ...
            abilities::enters_trigger(
                "When this creature enters, detain each nonland permanent \
...
// Audit: unsupported — The delayed trigger can retain bindings, but zone-move result bindings
// capture intended recipients before replacements rather than exactly the creatures
// successfully exiled together.
...
// Audit: unsupported — The unblocked-attacker trigger can set the defending player's life total
// and suppress this creature's combat damage, but no attack restriction can require this
// creature to attack alone.
        ...
        .with_abilities(&[
            ...
            AbilityDef::triggered(
                "Whenever you cast an instant or sorcery spell from your \
                 library, copy it. You may choose new targets for the copy.",
...
// Audit: unsupported — Needs reveal-until-four-matching-cards library traversal and moving the
// entire revealed group to the graveyard.
...
pub(in crate::card::sets) static MORGUE_BURST: CardRecord = CardRecord::new(
    ...
    CardRules::new_sorcery(mana_cost!("{4}{B}{R}")).with_ability(AbilityDef::spell_with_targets(
        "Return target creature card from your graveyard to your \
         hand. Morgue Burst deals damage to any target equal to the \
         power of the card returned this way.",
        &[
            AbilityTargetDef::exactly_one(AbilityTargetPredicate::Object {
                object: ObjectPredicateDef::HasType(CardType::Creature),
...
pub(in crate::card::sets) static NOTION_THIEF: CardRecord = CardRecord::new(
    ...
    "Clint Cearley",
    CardRules::new_creature(mana_cost!("{2}{U}{B}"), &["Human", "Rogue"], 3, 1).with_abilities(&[
        abilities::flash(),
        AbilityDef::defined_replacement(
            "If an opponent would draw a card except the first one they \
             draw in each of their draw steps, instead you draw a card.",
            ...
            ReplacementEffectDef::Sequence(&[
                ...
                ReplacementEffectDef::Perform(&EffectDef::DrawCards {
                    recipient: EffectRecipientDef::Controller,
                    amount: ValueDef::Constant(1),
                }),
            ]),
...
// Audit: unsupported — Needs a successful-counter result whose mana value can be retained by a
// delayed first-main-phase mana effect; variable mana combinations themselves are supported.
    ...
    CardRules::new_creature(mana_cost!("{4}{G}{U}"), &["Shapeshifter"], 0, 0).with_ability(
        AbilityDef::replacement(
            "You may have this creature enter as a copy of any creature \
             on the battlefield, except it has \"At the beginning of \
             your upkeep, if this creature isn't a token, create a token \
            ...
            crate::card::ReplacementEffectDef::CopyEntering {
                ...
                exceptions: CopyExceptionsDef::NONE.with_abilities(&[CopyAbilityDef::Ability(
                    &AbilityDef::triggered_if(
                        "At the beginning of your upkeep, if this creature isn't a \
                         token, create a token that's a copy of this creature.",
                        TriggerEventDef::StepBegins {
                            step: TurnStepDef::Upkeep,
                            player: PlayerRelation::You,
                        },
...
// Audit: unsupported — ChooseExact cannot model the optional “up to X” private-hand choice; the
// chosen-name union must then drive exiles across three zones and one final shuffle.
...
pub(in crate::card::sets) static RESTORE_THE_PEACE: CardRecord = CardRecord::new(
    ...
    "Kev Walker",
    CardRules::new_instant(mana_cost!("{1}{W}{U}")).with_ability(AbilityDef::spell(
...
pub(in crate::card::sets) static ROT_FARM_SKELETON: CardRecord = CardRecord::new(
    ...
    "Maciej Kuciara",
    CardRules::new_creature(mana_cost!("{2}{B}{G}"), &["Plant", "Skeleton"], 4, 1).with_abilities(
        &[
            AbilityDef::static_ability(
            ...
            AbilityDef::activated(
                "{2}{B}{G}, Mill four cards: Return this card from your \
...
pub(in crate::card::sets) static RURIC_THAR_THE_UNBOWED: CardRecord = CardRecord::new(
    ...
    "Tyler Jacobson",
    CardRules::new_creature(mana_cost!("{4}{R}{G}"), &["Ogre", "Warrior"], 6, 6)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            abilities::vigilance(),
            abilities::reach(),
            abilities::attacks_each_combat_if_able()
                .override_text("Ruric Thar attacks each combat if able."),
            AbilityDef::triggered(
                "Whenever a player casts a noncreature spell, Ruric Thar \
                 deals 6 damage to that player.",
...
pub(in crate::card::sets) static SAVAGEBORN_HYDRA: CardRecord = CardRecord::new(
    ...
    "Raymond Swanland",
    CardRules::new_creature(mana_cost!("{X}{R}{G}"), &["Hydra"], 0, 0).with_abilities(&[
...
// Audit: unsupported — Fight is available, but the trigger still cannot lock a uniformly random
// legal opponent-creature target as it is put on the stack.
...
pub(in crate::card::sets) static SHOWSTOPPER: CardRecord = CardRecord::new(
    ...
    CardRules::new_instant(mana_cost!("{1}{B}{R}")).with_ability(AbilityDef::spell(
        "Until end of turn, creatures you control gain ‘When this \
         creature dies, it deals 2 damage to target creature an \
         opponent controls.’",
        EffectDef::Apply {
            recipient: EffectRecipientDef::matching_objects(
                ObjectPredicateDef::HasType(CardType::Creature),
            ...
            effect: AppliedEffectDef::add_ability(&abilities::dies_trigger_with_targets(
                "When this creature dies, it deals 2 damage to target \
...
pub(in crate::card::sets) static SIN_COLLECTOR: CardRecord = CardRecord::new(
    ...
    "Mike Bierek",
    CardRules::new_creature(mana_cost!("{1}{W}{B}"), &["Human", "Cleric"], 2, 1).with_abilities(&[
        abilities::enters_trigger_with_targets(
            "When this creature enters, target opponent reveals their \
             hand. You choose an instant or sorcery card from it and \
             exile that card.",
            &[AbilityTargetDef::exactly_one(
                AbilityTargetPredicate::Player(PlayerRelation::Opponent),
            )],
...
pub(in crate::card::sets) static TEYSA_ENVOY_OF_GHOSTS: CardRecord = CardRecord::new(
    ...
    "Karla Ortiz",
    CardRules::new_creature(mana_cost!("{5}{W}{B}"), &["Human", "Advisor"], 4, 4)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            abilities::vigilance(),
            AbilityDef::keyword(
            ...
            AbilityDef::triggered(
                "Whenever a creature deals combat damage to you, destroy \
                 that creature. Create a 1/1 white and black Spirit creature \
                 token with flying.",
...
pub(in crate::card::sets) static TROSTANIS_SUMMONER: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{5}{G}{W}"), &["Elf", "Shaman"], 1, 1).with_ability(
        abilities::enters_trigger(
            "When this creature enters, create a 2/2 white Knight \
             creature token with vigilance, a 3/3 green Centaur creature \
             token, and a 4/4 green Rhino creature token with trample.",
            EffectDef::Sequence(&[
                EffectDef::CreateToken(CreateTokenDef::new(TokenDef::Literal(KNIGHT_TOKEN))),
...
// Audit: unsupported — Regeneration is available, but granting scavenge to every creature card
// in your graveyard still needs each card's own mana cost and power.
...
pub(in crate::card::sets) static VOICE_OF_RESURGENCE: CardRecord = CardRecord::new(
    ...
    "Winona Nelson",
    CardRules::new_creature(mana_cost!("{G}{W}"), &["Elemental"], 2, 2)
        // One printed sentence, two separate triggers: the cast one only during
        // your turn, and the death one whenever it happens.
        .with_abilities(&[
            AbilityDef::triggered_if(
                "Whenever an opponent casts a spell during your turn, create \
                 a green and white Elemental creature token with \"This \
            ...
            abilities::dies_trigger(
                "When this creature dies, create a green and white Elemental \
                 creature token with \"This token's power and toughness are \
...
// Audit: unsupported — DoubleCounters handles one named kind, not every counter kind present on
// the targeted artifact, creature, or land.
...
pub(in crate::card::sets) static WARPED_PHYSIQUE: CardRecord = CardRecord::new(
    ...
    "Karl Kopinski",
    CardRules::new_instant(mana_cost!("{U}{B}")).with_ability(AbilityDef::spell_with_targets(
        ...
        EffectDef::Apply {
            ...
            effect: AppliedEffectDef::modify_power_toughness(
                ...
                ValueDef::Negate(&ValueDef::CardsInHandAbove {
                    player: PlayerRelation::You,
                    threshold: 0,
                }),
            ),
...
pub(in crate::card::sets) static ZHUR_TAA_ANCIENT: CardRecord = CardRecord::new(
    ...
    "Adam Paquette",
    CardRules::new_creature(mana_cost!("{3}{R}{G}"), &["Beast"], 7, 5).with_ability(
...
// DGM 121 — Alive // Well
pub(in crate::card::sets) static ALIVE_WELL: CardRecord = CardRecord::new_fuse(
    "Alive // Well",
    ...
    &[
        ...
        (
            "Well",
            CardRules::new_sorcery(mana_cost!("{W}")).with_ability(AbilityDef::spell(
...
pub(in crate::card::sets) static ARMED_DANGEROUS: CardRecord = CardRecord::new_fuse(
    "Armed // Dangerous",
    ...
    &[
        ...
        (
            "Dangerous",
            CardRules::new_sorcery(mana_cost!("{3}{G}")).with_ability(
                AbilityDef::spell_with_targets(
...
// DGM 123 — Beck // Call
pub(in crate::card::sets) static BECK_CALL: CardRecord = CardRecord::new_fuse(
    "Beck // Call",
    ...
    &[
        (
            "Beck",
            CardRules::new_sorcery(mana_cost!("{G}{U}")).with_ability(AbilityDef::spell(
...
pub(in crate::card::sets) static BREAKING_ENTERING: CardRecord = CardRecord::new_fuse(
    "Breaking // Entering",
    ...
    &[
        (
            "Breaking",
            CardRules::new_sorcery(mana_cost!("{U}{B}")).with_ability(
                AbilityDef::spell_with_targets(
        ...
        (
            "Entering",
            CardRules::new_sorcery(mana_cost!("{4}{B}{R}")).with_ability(AbilityDef::spell(
                "Put a creature card from a graveyard onto the battlefield \
                 under your control. It gains haste until end of turn.",
                EffectDef::Choose(ChooseDef {
                    binding: ObjectChoiceBindingDef::Object(ParentBinding),
                    unchosen: None,
                    chooser: PlayerRefDef::EffectController,
...
pub(in crate::card::sets) static CATCH_RELEASE: CardRecord = CardRecord::new_fuse(
    "Catch // Release",
    ...
    &[
        (
            "Catch",
            CardRules::new_sorcery(mana_cost!("{1}{U}{R}")).with_ability(
                AbilityDef::spell_with_targets(
        ...
        (
            "Release",
            CardRules::new_sorcery(mana_cost!("{4}{R}{W}")).with_ability(AbilityDef::spell(
                "Each player sacrifices an artifact, a creature, an \
                 enchantment, a land, and a planeswalker of their choice.",
...
// DGM 128 — Flesh // Blood
pub(in crate::card::sets) static FLESH_BLOOD: CardRecord = CardRecord::new_fuse(
    "Flesh // Blood",
    ...
    &[
        (
            "Flesh",
            CardRules::new_sorcery(mana_cost!("{3}{B}{G}")).with_ability(
                AbilityDef::spell_with_targets(
                    "Exile target creature card from a graveyard. Put X +1/+1 \
                     counters on target creature, where X is the power of the \
                     card you exiled.",
                    &[
                        AbilityTargetDef::exactly_one(AbilityTargetPredicate::Object {
                            object: ObjectPredicateDef::HasType(CardType::Creature),
                    ...
                    EffectDef::Sequence(&[
                        ...
                        EffectDef::AddCounters {
                            object: EffectRecipientDef::Target(TargetIndex(1)),
                            kind: CounterKind::PlusOnePlusOne,
                            amount: ValueDef::ObjectPower(ObjectRefDef::Target(
                                TargetIndex::PRIMARY,
                            )),
                        },
                    ]),
...
// DGM 129 — Give // Take
// Audit: unsupported — RemoveAllCounters does not expose how many +1/+1 counters it removed for
// the fused Take half's draw instruction.
    ...
    &[
        (
            ...
            CardRules::new_instant(mana_cost!("{1}{G}{W}")).with_ability(AbilityDef::spell(
                "Creatures you control gain indestructible until end of \
...
// Audit: unsupported — Needs a value for the targeted player's current hand size; the available
// hand-size value is source-controller-relative.
    ...
    &[
            ...
            CardRules::new_instant(mana_cost!("{2}{U}")).with_ability(
                AbilityDef::spell_with_targets(
                    "Until end of turn, target creature loses all abilities and \
                     becomes a red Weird with base power and toughness \
        ...
        (
            ...
            CardRules::new_instant(mana_cost!("{1}{R}")).with_ability(
                AbilityDef::spell_with_targets(
                    "Burn deals 2 damage to any target.\nFuse (You may cast one \
                     or both halves of this card from your hand.)",
                    &[AbilityTargetDef::exactly_one(
                        AbilityTargetPredicate::AnyTarget,
                    )],
                    EffectDef::damage(
...
pub(in crate::card::sets) static MAZE_S_END: CardRecord = CardRecord::new(
    ...
    CardRules::new_land(&[]).with_abilities(&[
        ...
        AbilityDef::activated(
            "{3}, {T}, Return this land to its owner's hand: Search your \
             library for a Gate card, put it onto the battlefield, then \
             shuffle. If you control ten or more Gates with different \
