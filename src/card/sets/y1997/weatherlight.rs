...
pub(in crate::card::sets) static ABEYANCE: CardRecord = CardRecord::new(
    ...
    CardRules::new_instant(mana_cost!("{1}{W}")).with_ability(AbilityDef::spell_with_targets(
        "Until end of turn, target player can't cast instant or \
         sorcery spells, and that player can't activate abilities \
...
// WTH 3 — Alms
// Audit: unsupported — Needs an activation cost that exiles specifically the top card of a
// graveyard.
...
pub(in crate::card::sets) static ANGELIC_RENEWAL: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{1}{W}")).with_ability(AbilityDef::triggered(
        "Whenever a creature is put into your graveyard from the \
         battlefield, you may sacrifice this enchantment. If you do, \
...
// Audit: unsupported — Needs a turn-long combat-damage prevention shield keyed to one targeted
// blocked creature.
...
// Audit: unsupported — Needs to gain control only when this spell's regeneration shield
// actually replaces destruction.
...
// Audit: unsupported — Needs a power-based blocking restriction and a chosen-source prevention
// shield for the attached creature.
...
pub(in crate::card::sets) static MISTMOON_GRIFFIN: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{3}{W}"), &["Griffin"], 2, 2).with_abilities(&[
        ...
        abilities::dies_trigger(
            "When this creature dies, exile it, then return the top \
...
// Audit: unsupported — Leave-the-battlefield triggers cannot yet read the source's last-known
// counter count.
...
// WTH 28 — Tariff
// Audit: unsupported — Needs each player to choose a greatest-mana-value creature, then pay
// that object's full mana cost or sacrifice it.
...
// WTH 30 — Abduction
// Audit: unsupported — Needs an Aura-host death trigger that returns that exact zone-change
// successor under its owner's control.
...
pub(in crate::card::sets) static ABJURE: CardRecord = CardRecord::new(
    ...
    "Ted Naifeh",
    CardRules::new_instant(mana_cost!("{U}")).with_ability(AbilityDef::spell_with_additional_cost(
        "As an additional cost to cast this spell, sacrifice a blue \
...
// Audit: unsupported — Needs ordered top-ten inspection with a chosen subset exiled and the
// remainder reordered on top.
...
// WTH 33 — Apathy
// Audit: unsupported — Needs random discard as an optional effect payment by the attached
...
// Audit: unsupported — Needs phasing, phase-transition triggers, and a turn-bounded effect that
// suppresses the next phase-out.
...
pub(in crate::card::sets) static MANA_CHAINS: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{U}"))
        ...
        .with_abilities(&[
            abilities::aura_spell("Enchant creature", &abilities::ENCHANT_CREATURE_TARGET),
            AbilityDef::static_ability(
                "Enchanted creature has \"Cumulative upkeep {1}.\" (At the \
                 beginning of its controller's upkeep, that player puts an \
                 age counter on it, then sacrifices it unless they pay its \
...
// WTH 42 — Manta Ray
// Audit: unsupported — Needs Island-dependent attack permission, blue-only blocking, and the
// no-Island state trigger.
...
// Audit: unsupported — Needs APNAP optional library searches with independent searched-player
// tracking and shuffles.
...
// WTH 45 — Ophidian
// Audit: unsupported — Needs an unblocked-attacker choice that suppresses this creature's
// combat-damage assignment when accepted.
...
// Audit: unsupported — Needs phasing plus a delayed end-of-combat phase-out for each attacking
// creature.
...
// Audit: unsupported — Needs to bind each individual blocking creature and gain control of it
// at end of combat.
...
// Audit: unsupported — Needs hidden-hand selection of two cards followed by an ordered move to
// the top of that player's library.
...
// WTH 61 — Barrow Ghoul
// Audit: unsupported — Needs an upkeep payment that exiles specifically the top creature card
// of a graveyard.
...
// WTH 62 — Bone Dancer
// Audit: unsupported — Needs an unblocked-attacker choice, defending-player graveyard top-card
// lookup, and combat-damage suppression.
...
// Audit: unsupported — Needs a hand-zone special action and an upkeep payment that exiles the
// top creature card of a graveyard.
...
// Audit: unsupported — Needs the set of non-Wall creatures currently blocking the attached
// creature.
...
pub(in crate::card::sets) static DOOMSDAY: CardRecord = CardRecord::new(
    ...
    CardRules::new_sorcery(mana_cost!("{B}{B}{B}")).with_ability(AbilityDef::spell(
        "Search your library and graveyard for five cards and exile \
         the rest. Put the chosen cards on top of your library in \
         any order. You lose half your life, rounded up.",
        ...
        EffectDef::Sequence(&[
            ...
            EffectDef::LoseLife {
                ...
                amount: ValueDef::Halved(&HalvedValueDef::new(
                    ValueDef::LifeTotal(PlayerRelation::You),
                    RoundingDef::Up,
                )),
            },
        ]),
...
pub(in crate::card::sets) static FESTERING_EVIL: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{3}{B}{B}")).with_abilities(&[
        AbilityDef::triggered(
            "At the beginning of your upkeep, this enchantment deals 1 \
        ...
        AbilityDef::activated(
            "{B}{B}, Sacrifice this enchantment: It deals 3 damage to \
...
pub(in crate::card::sets) static HAUNTING_MISERY: CardRecord = CardRecord::new(
    ...
    CardRules::new_sorcery(mana_cost!("{1}{B}{B}")).with_ability(
        AbilityDef::spell_with_additional_cost(
            "As an additional cost to cast this spell, exile X creature \
             cards from your graveyard.\nThis spell deals X damage to \
...
// WTH 76 — Necratog
// Audit: unsupported — Needs an activation cost that exiles specifically the top creature card
// of a graveyard.
...
// Audit: unsupported — Needs an alternative cost that exiles the top three black cards of the
// caster's graveyard.
...
pub(in crate::card::sets) static STRANDS_OF_NIGHT: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{2}{B}{B}")).with_ability(
        AbilityDef::activated_with_targets(
            "{B}{B}, Pay 2 life, Sacrifice a Swamp: Return target \
...
// Audit: unsupported — Needs the count of creatures put into the caster's graveyard from the
...
// Audit: unsupported — Needs an each-player upkeep condition and damage recipient tied to that
...
pub(in crate::card::sets) static WAVE_OF_TERROR: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{2}{B}")).with_abilities(&[
        abilities::cumulative_upkeep(&[CostDef::mana(mana_cost!("{1}"))]),
        AbilityDef::triggered(
            "At the beginning of your draw step, destroy each creature \
...
pub(in crate::card::sets) static CINDER_GIANT: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{3}{R}"), &["Giant"], 5, 3).with_ability(
        AbilityDef::triggered(
            "At the beginning of your upkeep, this creature deals 2 \
...
// WTH 94 — Cinder Wall
// Audit: unsupported — Needs a delayed end-of-combat destruction effect bound to the blocking
// event.
...
pub(in crate::card::sets) static CONE_OF_FLAME: CardRecord = CardRecord::new(
    ...
    CardRules::new_sorcery(mana_cost!("{3}{R}{R}")).with_ability(AbilityDef::spell_with_targets(
        "This spell deals 1 damage to any target, 2 damage to \
         another target, and 3 damage to a third target.",
        &[
...
// Audit: unsupported — Needs a chosen source, a coin flip, and a consumable next-damage
// replacement selected by the flip result.
...
pub(in crate::card::sets) static DWARVEN_BERSERKER: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{1}{R}"), &["Dwarf", "Berserker"], 1, 1).with_ability(
        AbilityDef::triggered(
            "Whenever this creature becomes blocked, it gets +3/+0 and \
...
pub(in crate::card::sets) static FIRESTORM: CardRecord = CardRecord::new(
    ...
    "Jeff Miracola",
    CardRules::new_instant(mana_cost!("{R}")).with_ability(AbilityDef::spell_with_additional_cost(
...
// WTH 103 — Goblin Bomb
// Audit: unsupported — Needs coin-flip branching and an activation cost that removes exactly
// five fuse counters atomically.
...
// Audit: unsupported — Needs an unblocked-attacker sacrifice choice followed by independently
// targeted creature and land destruction.
    ...
    CardRules::new_creature(mana_cost!("{R}"), &["Goblin", "Rogue"], 1, 1).with_ability(
        AbilityDef::triggered_with_targets(
            "Whenever this creature attacks and isn't blocked, you may \
             pay {R}. If you do, destroy target artifact defending \
             player controls and this creature assigns no combat damage \
             this turn.",
    ...
    CardRules::new_enchantment(mana_cost!("{2}{R}{R}")).with_abilities(&[
        ...
        AbilityDef::triggered_with_targets(
            "When a player doesn't pay this enchantment's cumulative \
             upkeep, this enchantment deals X damage to target player or \
             planeswalker and each creature that player or that \
             planeswalker's controller controls, where X is twice the \
             number of age counters on this enchantment minus 2.",
...
// WTH 107 — Heat Stroke
// Audit: unsupported — Needs end-of-combat history for every creature that blocked or was
// blocked this turn.
...
// Audit: unsupported — Needs a characteristic-defining count of untapped artifacts, creatures,
// and lands controlled by the source's controller.
...
// Audit: unsupported — The runtime cannot yet enumerate or pay a repeated-X activation cost
// while using that X as the exact target count.
...
// Audit: unsupported — Needs delayed end-of-combat damage to each creature paired with this
// source by blocking.
...
pub(in crate::card::sets) static BARISHI: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{2}{G}{G}"), &["Elemental"], 4, 3).with_ability(
        abilities::dies_trigger(
            "When this creature dies, exile it, then shuffle all \
...
pub(in crate::card::sets) static CALL_OF_THE_WILD: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{2}{G}{G}")).with_ability(AbilityDef::activated(
        "{2}{G}{G}: Reveal the top card of your library. If it's a \
         creature card, put it onto the battlefield. Otherwise, put \
...
// Audit: unsupported — Needs declare-blockers-only cast timing and a rule effect that marks
// targeted attackers blocked without blockers.
...
// Audit: unsupported — Needs a spell-only targeting prohibition; hexproof/shroud would
// incorrectly stop abilities too.
...
pub(in crate::card::sets) static FUNGUS_ELEMENTAL: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{3}{G}"), &["Fungus", "Elemental"], 3, 3).with_ability(
        AbilityDef::activated(
            "{G}, Sacrifice a Forest: Put a +2/+2 counter on this \
             creature. Activate only if this creature entered this turn.",
            &[
                CostDef::Mana(mana_cost!("{G}")),
                CostDef::SacrificePermanent {
...
// Audit: unsupported — Needs graveyard targets constrained to a separately targeted player's
// graveyard plus ordered library insertion.
...
// Audit: unsupported — Needs an enters payment that moves a chosen basic land card from the
...
// Audit: unsupported — Needs APNAP chosen-any-amount mana payments with per-player paid amounts
// retained for token creation.
...
pub(in crate::card::sets) static LLANOWAR_SENTINEL: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{2}{G}"), &["Elf"], 2, 3).with_ability(
        abilities::enters_trigger(
            "When this creature enters, you may pay {1}{G}. If you do, \
             search your library for a card named Llanowar Sentinel, put \
...
pub(in crate::card::sets) static MWONVULI_OOZE: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{G}"), &["Ooze"], 0, 0).with_abilities(&[
        abilities::cumulative_upkeep(&[CostDef::mana(mana_cost!("{2}"))]).override_text(
            "Cumulative upkeep {2} (At the beginning of your upkeep, put \
             an age counter on this permanent, then sacrifice it unless \
             you pay {2} for each age counter on it.)",
        ),
        AbilityDef::static_ability(
            "This creature's power and toughness are each equal to 1 \
...
// Audit: unsupported — Needs per-player creature-card counts to feed distinct draw amounts
// during one resolution.
...
// Audit: unsupported — Needs a dies trigger that excludes its own zone-change result from the
// graveyard target set.
...
// Audit: unsupported — Needs APNAP optional searches whose chosen cards enter tapped and whose
// searched players shuffle independently.
...
// Audit: unsupported — Needs cast permission restricted to the current top instant or sorcery
// card of the graveyard plus linked exile replacement.
...
pub(in crate::card::sets) static CHIMERIC_SPHERE: CardRecord = CardRecord::new(
    ...
    CardRules::new_artifact(mana_cost!("{3}")).with_abilities(&[
        AbilityDef::activated(
            "{2}: Until end of turn, this artifact becomes a 2/1 \
            ...
            EffectDef::Apply {
                ...
                effect: AppliedEffectDef::Composite(&[
                    ...
                    AppliedEffectDef::set_creature_types(CreatureTypeSetDef::named(&["Construct"])),
        ...
        AbilityDef::activated(
            "{2}: Until end of turn, this artifact becomes a 3/2 \
             Construct artifact creature and loses flying.",
            &[CostDef::Mana(mana_cost!("{2}"))],
            EffectDef::Apply {
                recipient: EffectRecipientDef::Source,
                effect: AppliedEffectDef::Composite(&[
...
// Audit: unsupported — Needs the defending player's identity for an attack trigger that untaps
// all creatures they control.
...
// WTH 152 — Mana Web
// Audit: unsupported — Needs mana-production equivalence between a triggering land and every
// other land the same opponent controls.
...
pub(in crate::card::sets) static PHYREXIAN_FURNACE: CardRecord = CardRecord::new(
    ...
    CardRules::new_artifact(mana_cost!("{1}")).with_abilities(&[
        AbilityDef::activated_with_targets(
            ...
            EffectDef::move_to_zone(
                EffectRecipientDef::objects(ObjectSetDef::BottomOfGraveyard(PlayerRefDef::Target(
                    TargetIndex::PRIMARY,
                ))),
                ZoneKind::Exile,
                ZonePlacement::Top,
...
pub(in crate::card::sets) static THRAN_FORGE: CardRecord = CardRecord::new(
    ...
    CardRules::new_artifact(mana_cost!("{3}")).with_ability(AbilityDef::activated_with_targets(
        "{2}: Until end of turn, target nonartifact creature gets \
         +1/+0 and becomes an artifact in addition to its other \
         types.",
        &[CostDef::Mana(mana_cost!("{2}"))],
        &[AbilityTargetDef::exactly_one_permanent(
            ObjectPredicateDef::All(&[
...
// WTH 160 — Thran Tome
// Audit: unsupported — Needs ordered top-three reveal, an opponent's choice from that revealed
// group, and drawing the remainder.
...
// Audit: unsupported — Needs an activation restriction tied specifically to each player's draw
// step.
...
pub(in crate::card::sets) static XANTHIC_STATUE: CardRecord = CardRecord::new(
    ...
    CardRules::new_artifact(mana_cost!("{8}")).with_ability(AbilityDef::activated(
        "{5}: Until end of turn, this artifact becomes an 8/8 Golem \
         artifact creature with trample.",
...
pub(in crate::card::sets) static GEMSTONE_MINE: CardRecord = CardRecord::new(
    ...
    CardRules::new_land(&[]).with_abilities(&[
        ...
        AbilityDef::activated_mana(
            "{T}, Remove a mining counter from this land: Add one mana \
             of any color. If there are no mining counters on this land, \
             sacrifice it.",
            &[
                CostDef::TapSource,
            ...
            EffectDef::AddMana(
                AddManaEffectDef::any_color()
                    .sacrificing_source_when_out_of(CounterKind::named("mining")),
            ),
        ),
    ]),
);
...
// WTH 165 — Lotus Vale
// Audit: unsupported — Needs an enters replacement that atomically sacrifices two untapped
// lands or moves this land to its owner's graveyard.
