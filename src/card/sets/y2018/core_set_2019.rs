...
abilities::enters_trigger("When Lena enters, create a 1/1 white Soldier creature token for each nontoken creature you control.", EffectDef::CreateToken(crate::card::CreateTokenDef::new(crate::card::TokenDef::Literal(crate::card::TokenCharacteristics::creature(&["Soldier"], &[ManaColor::White], 1, 1))).with_count(ValueDef::CountMatchingObjects(&ObjectQueryDef::matching(ObjectPredicateDef::All(&[ObjectPredicateDef::HasType(CardType::Creature), ObjectPredicateDef::Not(&ObjectPredicateDef::Token)]), &[ZoneKind::Battlefield], PlayerRelation::You))))),
