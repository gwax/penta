    ...
    CardRules::new_creature(mana_cost!("{3}{U}"), &["Human", "Wizard"], 1, 5).with_ability(
        AbilityDef::triggered(
            "Whenever you cast an instant or sorcery spell, create a 1/1 \
             blue Bird Illusion creature token with flying.",
...
pub(in crate::card::sets) static MAUSOLEUM_SECRETS: CardRecord = CardRecord::new(
    ...
    CardRules::new_instant(mana_cost!("{1}{B}")).with_abilities(&[AbilityDef::spell(
        "Undergrowth — Search your library for a black card with \
         mana value less than or equal to the number of creature \
...
pub(in crate::card::sets) static ARCLIGHT_PHOENIX: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{3}{R}"), &["Phoenix"], 3, 2).with_abilities(&[
        ...
        AbilityDef::triggered_if(
            "At the beginning of combat on your turn, if you've cast \
             three or more instant and sorcery spells this turn, return \
...
pub(in crate::card::sets) static GOBLIN_CRATERMAKER: CardRecord = CardRecord::new(
    "Goblin Cratermaker",
...
pub(in crate::card::sets) static LEGION_WARBOSS: CardRecord = CardRecord::new(
    "Legion Warboss",
    "5e84cb9c-9876-47a4-aea4-78574321bc36",
    "Alex Konstad",
    CardRules::new_creature(mana_cost!("{2}{R}"), &["Goblin", "Soldier"], 2, 2).with_abilities(&[
        AbilityDef::triggered_with_targets(
            "Mentor (Whenever this creature attacks, put a +1/+1 counter \
        ...
        AbilityDef::triggered(
            "At the beginning of combat on your turn, create a 1/1 red \
             Goblin creature token. That token gains haste until end of \
             turn and attacks this combat if able.",
            TriggerEventDef::StepBegins {
                step: TurnStepDef::BeginningOfCombat,
                player: PlayerRelation::You,
            },
...
pub(in crate::card::sets) static RUNAWAY_STEAM_KIN: CardRecord = CardRecord::new(
    "Runaway Steam-Kin",
    "d8c9c111-fbc7-44e1-94bd-1ca164370623",
    "Jason Felix",
    CardRules::new_creature(mana_cost!("{1}{R}"), &["Elemental"], 1, 1).with_abilities(&[
        AbilityDef::triggered_if(
            "Whenever you cast a red spell, if this creature has fewer \
             than three +1/+1 counters on it, put a +1/+1 counter on \
             this creature.",
            TriggerEventDef::spell_cast(ObjectPredicateDef::All(&[
        ...
        AbilityDef::activated_mana(
            "Remove three +1/+1 counters from this creature: Add \
             {R}{R}{R}.",
...
pub(in crate::card::sets) static TORCH_COURIER: CardRecord = CardRecord::new(
    "Torch Courier",
...
// Audit: unsupported — Arbitrary-card exile grants cannot attach a spend-mana-as-any-type
// permission to the exiled graveyard group. That modifier is exposed only on top-of-library
// exile operations, which cannot select these cards.
pub(in crate::card::sets) static MNEMONIC_BETRAYAL: CardRecord = CardRecord::new(
    "Mnemonic Betrayal",
...
pub(in crate::card::sets) static NIV_MIZZET_PARUN: CardRecord = CardRecord::new(
    "Niv-Mizzet, Parun",
...
// Audit: unsupported — Needs a cast-event snapshot counting prior instant and sorcery spells
// this turn; the current matching-spell count reads live history at resolution and can include
// spells cast after this trigger.
...
pub(in crate::card::sets) static INVERT_INVENT: CardRecord = CardRecord::new_split(
    "Invert // Invent",
    ...
    &[
        (
            ...
            CardRules::new_instant(mana_cost!("{U/R}")).with_ability(
                AbilityDef::spell_with_targets(
                    "Switch the power and toughness of each of up to two target \
        ...
        (
            "Invent",
            CardRules::new_instant(mana_cost!("{4}{U}{R}")).with_ability(AbilityDef::spell(
                "Search your library for an instant card and/or a sorcery \
                 card, reveal them, put them into your hand, then shuffle.",
                EffectDef::Sequence(&[
                    EffectDef::ChooseOneOfEach(ChooseOneOfEachDef {
                        ...
                        chosen: Binding!("invent_found"),
                        remainder: Binding!("invent_rest"),
...
pub(in crate::card::sets) static WAND_OF_VERTEBRAE: CardRecord = CardRecord::new(
    "Wand of Vertebrae",
    ...
    CardRules::new_artifact(mana_cost!("{1}")).with_abilities(&[
        ...
        AbilityDef::activated_with_targets(
            "{2}, {T}, Exile this artifact: Shuffle up to five target \
