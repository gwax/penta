...
pub(in crate::card::sets) static CERULEAN_WISPS: CardRecord = CardRecord::new(
    "Cerulean Wisps",
...
pub(in crate::card::sets) static COUNTERBORE: CardRecord = CardRecord::new(
    ...
    CardRules::new_instant(mana_cost!("{3}{U}{U}")).with_ability(AbilityDef::spell_with_targets(
        "Counter target spell. Search its controller's graveyard, \
         hand, and library for all cards with the same name as that \
         spell and exile them. Then that player shuffles.",
...
pub(in crate::card::sets) static KNACKSAW_CLIQUE: CardRecord = CardRecord::new(
    "Knacksaw Clique",
...
pub(in crate::card::sets) static FAERIE_MACABRE: CardRecord = CardRecord::new(
    "Faerie Macabre",
...
pub(in crate::card::sets) static CRIMSON_WISPS: CardRecord = CardRecord::new(
    "Crimson Wisps",
    "a65c81ff-fc5d-4191-93fb-52eb806457b7",
    "Jim Nelson",
    CardRules::new_instant(mana_cost!("{R}")).with_abilities(&[AbilityDef::spell_with_targets(
        "Target creature becomes red and gains haste until end of \
         turn. (It can attack and {T} this turn.)\nDraw a card.",
        &[AbilityTargetDef::exactly_one_permanent(
            ObjectPredicateDef::HasType(CardType::Creature),
        )],
        EffectDef::Sequence(&[
...
// the stack.
pub(in crate::card::sets) static DEVOTED_DRUID: CardRecord = CardRecord::new(
    "Devoted Druid",
    ...
    CardRules::new_creature(mana_cost!("{5}{G}{G}"), &["Troll"], 5, 5).with_abilities(&[
        ...
        AbilityDef::activated(
            "Tap any number of untapped creatures you control other than \
             this creature with total power 10 or greater: This creature \
...
pub(in crate::card::sets) static HELM_OF_THE_GHASTLORD: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{3}{U/B}"))
        ...
        .with_abilities(&[
            ...
            AbilityDef::static_ability(
                "As long as enchanted creature is blue, it gets +1/+1 and \
                 has \"Whenever this creature deals damage to an opponent, \
                 draw a card.\"",
                    ...
                    then: &EffectDef::StaticApply {
                        ...
                        effect: AppliedEffectDef::Composite(&[
                            ...
                            AppliedEffectDef::add_ability(&AbilityDef::triggered(
                                "Whenever this creature deals damage to an opponent, draw a \
                                 card.",
            ...
            AbilityDef::static_ability(
                "As long as enchanted creature is black, it gets +1/+1 and \
                 has \"Whenever this creature deals damage to an opponent, \
...
pub(in crate::card::sets) static MURDEROUS_REDCAP: CardRecord = CardRecord::new(
    "Murderous Redcap",
...
pub(in crate::card::sets) static GUTTURAL_RESPONSE: CardRecord = CardRecord::new(
    "Guttural Response",
...
pub(in crate::card::sets) static SEEDCRADLE_WITCH: CardRecord = CardRecord::new(
    "Seedcradle Witch",
...
// Audit: unsupported — Needs a prospective discard event carrying the causing spell or
// ability's controller, then replacing the discard destination with battlefield entry; a
// generic graveyard move cannot distinguish discard from other moves.
...
// walks.
pub(in crate::card::sets) static PAINTER_S_SERVANT: CardRecord = CardRecord::new(
    "Painter's Servant",
...
// SHM 258 — Pili-Pala
// Audit: unsupported — The mana-ability eligibility/payment path rejects UntapSource costs;
// ordinary nonmana untap-symbol activations do not make this mana ability executable.
pub(in crate::card::sets) static PILI_PALA: CardRecord = CardRecord::new(
    "Pili-Pala",
...
pub(in crate::card::sets) static UMBRAL_MANTLE: CardRecord = CardRecord::new(
    "Umbral Mantle",
        ...
        .with_abilities(&[
            AbilityDef::static_ability(
                "Equipped creature has \"{3}, {Q}: This creature gets +2/+2 \
                 until end of turn.\" ({Q} is the untap symbol.)",
                EffectDef::StaticApply {
                    ...
                    effect: AppliedEffectDef::add_ability(&AbilityDef::activated(
                        ...
                        &[CostDef::Mana(mana_cost!("{3}")), CostDef::UntapSource],
                        EffectDef::Apply {
                            recipient: EffectRecipientDef::Source,
                            effect: AppliedEffectDef::modify_power_toughness(
                                ValueDef::Constant(2),
...
pub(in crate::card::sets) static FIRE_LIT_THICKET: CardRecord = CardRecord::new(
    "Fire-Lit Thicket",
...
// Audit: unsupported — Mana-ability payment rejects hybrid symbols in activation costs; it
// cannot choose which half of the filter cost to pay.
pub(in crate::card::sets) static MYSTIC_GATE: CardRecord = CardRecord::new(
    "Mystic Gate",
...
// Audit: unsupported — Mana-ability payment rejects hybrid symbols in activation costs; it
// cannot choose which half of the filter cost to pay.
pub(in crate::card::sets) static SUNKEN_RUINS: CardRecord = CardRecord::new(
    "Sunken Ruins",
...
pub(in crate::card::sets) static WOODED_BASTION: CardRecord = CardRecord::new(
    "Wooded Bastion",
