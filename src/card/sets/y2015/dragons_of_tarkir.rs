...
// Audit: unsupported — Exploit must emit a distinct sacrifice-mechanic event and trigger only
// while Sidisi is still present. A sacrifice follow-up would incorrectly resolve the tutor in
// the entry trigger and even after sacrificing Sidisi herself; there is no exploit event
// matcher.
pub(in crate::card::sets) static SIDISI_UNDEAD_VIZIER: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static RENDING_VOLLEY: CardRecord = CardRecord::new(
    "Rending Volley",
...
// Audit: unsupported — Needs a battlefield-typed saved object group that the ability-grant
// validator can accept after drawing cards; plain object bindings are treated as possibly
// nonbattlefield and cannot receive temporary indestructible.
...
// Audit: unsupported — Needs the intervening-if evaluator to aggregate the total power of
// controlled creatures at both trigger creation and resolution; AggregateObjectValues currently
// works only during effect resolution.
...
// Audit: unsupported — Needs mana-retention expiry attached to the six mana units generated by
// this trigger; retention permissions currently apply to a player's pool rather than only those
// particular units.
                ...
                AbilityTargetPredicate::Object {
                    object: ObjectPredicateDef::AnyOf(&[
                        ...
                        ObjectPredicateDef::All(&[
                            ...
                            ObjectPredicateDef::Subtype(SubtypeDef::from_name("Ugin")),
                        ]),
                    ]),
                    zones: &[ZoneKind::Graveyard],
                    controller: None,
