...
pub(in crate::card::sets) static GHOSTLY_PRISON: CardRecord = CardRecord::new(
    "Ghostly Prison",
...
// Audit: unsupported — Library selection cannot require pairwise distinct card names before the
// opponent divides the revealed selection between hand and graveyard.
pub(in crate::card::sets) static GIFTS_UNGIVEN: CardRecord = CardRecord::new(
    "Gifts Ungiven",
...
// CHK 97 — Time Stop
// Audit: unsupported — Needs an end-the-turn rules procedure that exiles the stack, ends
// resolving procedures, skips directly to cleanup, and performs the special cleanup rules.
...
// Audit: unsupported — Needs a positive cast-from-hand entry condition; SourceNotCastFrom
// cannot distinguish an actual hand cast from a non-cast battlefield arrival when inverted by
// the current entry condition vocabulary.
...
// Audit: unsupported — Needs bushido plus a static legend-rule exemption gated on exactly two
// same-named permanents across the battlefield.
...
pub(in crate::card::sets) static DESPERATE_RITUAL: CardRecord = CardRecord::new(
    "Desperate Ritual",
...
pub(in crate::card::sets) static KIKI_JIKI_MIRROR_BREAKER: CardRecord = CardRecord::new(
    "Kiki-Jiki, Mirror Breaker",
    "162018eb-5483-4fa2-9c5a-abb639eecf91",
    "Pete Venters",
    CardRules::new_creature(mana_cost!("{2}{R}{R}{R}"), &["Goblin", "Shaman"], 2, 2)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            abilities::haste(),
            AbilityDef::activated_with_targets(
                "{T}: Create a token that's a copy of target nonlegendary \
                 creature you control, except it has haste. Sacrifice it at \
                ...
                EffectDef::CreateToken(
                    crate::card::CreateTokenDef::new(crate::card::TokenDef::Copy(&TokenCopyDef {
                        ...
                        exceptions: CopyExceptionsDef::NONE
                            .with_abilities(&[CopyAbilityDef::Ability(&abilities::haste())]),
                    }))
        ...
        .with_abilities(&[
            AbilityDef::spell(
                "You may put a creature card from your hand onto the \
                 battlefield. That creature gains haste. Sacrifice that \
                    ...
                    then: &EffectDef::Apply {
                        ...
                        effect: AppliedEffectDef::Composite(&[
                            ...
                            AppliedEffectDef::add_ability(&AbilityDef::triggered(
                                "At the beginning of the next end step, sacrifice this \
                                 creature.",
                                TriggerEventDef::StepBegins {
                                    step: TurnStepDef::End,
                                    player: PlayerRelation::Any,
            ...
            abilities::splice_onto_arcane(&[CostDef::Mana(mana_cost!("{2}{R}{R}"))]),
        ]),
);
...
pub(in crate::card::sets) static DOSAN_THE_FALLING_LEAF: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static KODAMA_S_REACH: CardRecord = CardRecord::new(
    "Kodama's Reach",
...
pub(in crate::card::sets) static TIME_OF_NEED: CardRecord = CardRecord::new(
    "Time of Need",
...
pub(in crate::card::sets) static OROCHI_HATCHERY: CardRecord = CardRecord::new(
    "Orochi Hatchery",
    ...
    CardRules::new_artifact(mana_cost!("{X}{X}")).with_abilities(&[
        ...
        AbilityDef::activated(
            "{5}, {T}: Create a 1/1 green Snake creature token for each \
...
pub(in crate::card::sets) static BOSEIJU_WHO_SHELTERS_ALL: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static EIGANJO_CASTLE: CardRecord = CardRecord::new(
    "Eiganjo Castle",
...
pub(in crate::card::sets) static FORBIDDEN_ORCHARD: CardRecord = CardRecord::new(
    "Forbidden Orchard",
...
pub(in crate::card::sets) static HALL_OF_THE_BANDIT_LORD: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static SHINKA_THE_BLOODSOAKED_KEEP: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static SHIZO_DEATH_S_STOREHOUSE: CardRecord = CardRecord::new(
        ...
        .with_abilities(&[
            ...
            AbilityDef::activated_with_targets(
                "{B}, {T}: Target legendary creature gains fear until end of \
                 turn. (It can't be blocked except by artifact creatures \
