...
pub(in crate::card::sets) static MARCH_OF_OTHERWORLDLY_LIGHT: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static DISRUPTION_PROTOCOL: CardRecord = CardRecord::new(
    "Disruption Protocol",
...
pub(in crate::card::sets) static MARCH_OF_SWIRLING_MIST: CardRecord = CardRecord::new(
...
// Audit: unsupported — Its mana activation requires a selected artifact-or-creature tap in
// addition to tapping the source, which the mana-ability planner rejects.
pub(in crate::card::sets) static MOONSNARE_PROTOTYPE: CardRecord = CardRecord::new(
    "Moonsnare Prototype",
...
// Audit: unsupported — The zone-change stream cannot aggregate simultaneous returns to hand
// into one event, and the activated-cost planner cannot return a selected land as this
// activation's cost.
pub(in crate::card::sets) static TAMESHI_REALITY_ARCHITECT: CardRecord = CardRecord::new(
...
// Audit: unsupported — Needs a Saga clause shared across chapters. Every piece is present --
// saga_chapter, exile_and_return_transformed, and the transforming two-face record -- but "I,
// II" is one printed clause on two chapters, and a chapter is one ability per lore counter.
// Repeating the text prints the line twice and leaving the second empty is rejected outright.
...
pub(in crate::card::sets) static EXPERIMENTAL_SYNTHESIZER: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static GORO_GORO_DISCIPLE_OF_RYUSEI: CardRecord = CardRecord::new(
    "Goro-Goro, Disciple of Ryusei",
    "1ca736c7-35a9-48c7-b5a9-69b2a6e33ad0",
    "Mike Jordana",
    CardRules::new_creature(mana_cost!("{1}{R}"), &["Goblin", "Samurai"], 2, 2)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            AbilityDef::activated(
            ...
            AbilityDef::activated(
                "{3}{R}{R}: Create a 5/5 red Dragon Spirit creature token \
                 with flying. Activate only if you control an attacking \
                 modified creature. (Equipment, Auras you control, and \
...
// Audit: unsupported — The casting planner cannot select an arbitrary red-card exile group and
// reduce this spell's generic cost by two per card. The exile-play permission also cannot limit
// the group to two plays.
pub(in crate::card::sets) static MARCH_OF_RECKLESS_JOY: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static RABBIT_BATTERY: CardRecord = CardRecord::new(
    ...
    CardRules::new_artifact_creature(mana_cost!("{R}"), &["Equipment", "Rabbit"], 1, 1)
        .with_abilities(&[
            ...
            abilities::reconfigure(
                &[CostDef::Mana(mana_cost!("{R}"))],
                "Reconfigure {R} ({R}: Attach to target creature you \
                 control; or unattach from a creature. Reconfigure only as a \
...
pub(in crate::card::sets) static TWINSHOT_SNIPER: CardRecord = CardRecord::new(
    "Twinshot Sniper",
...
pub(in crate::card::sets) static GREATER_TANUKI: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment_creature(mana_cost!("{4}{G}{G}"), &["Dog"], 6, 5).with_abilities(&[
        abilities::trample(),
        AbilityDef::activated(
            "Channel — {2}{G}, Discard this card: Search your library \
...
pub(in crate::card::sets) static HARMONIOUS_EMERGENCE: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{3}{G}"))
        ...
        .with_abilities(&[
            abilities::enchant_land_you_control(),
            AbilityDef::static_ability(
                "Enchanted land is a 4/5 green Spirit creature with \
                 vigilance and haste. It's still a land.",
                EffectDef::StaticApply {
                    recipient: EffectRecipientDef::AttachedPermanent,
                    effect: AppliedEffectDef::Composite(&[
                        AppliedEffectDef::add_card_types(CardTypeSet::single(CardType::Creature)),
...
pub(in crate::card::sets) static COLOSSAL_SKYTURTLE: CardRecord = CardRecord::new(
    "Colossal Skyturtle",
    "f40bd797-4d12-4098-a1a8-d7e5b7b82ac9",
    "Nicholas Gregory",
    CardRules::new_enchantment_creature(mana_cost!("{4}{G}{G}{U}"), &["Turtle"], 6, 5)
        .with_abilities(&[
            abilities::flying(),
            abilities::ward(&[CostDef::Mana(mana_cost!("{2}"))], "Ward {2}"),
            AbilityDef::activated_with_targets(
                "Channel — {2}{G}, Discard this card: Return target card \
            ...
            .with_source_zones(&[ZoneKind::Hand]),
            AbilityDef::activated_with_targets(
                "Channel — {1}{U}, Discard this card: Return target creature \
                 to its owner's hand.",
                &[CostDef::Mana(mana_cost!("{1}{U}")), CostDef::DiscardSource],
                &[AbilityTargetDef::exactly_one_permanent(
                    ObjectPredicateDef::HasType(CardType::Creature),
                )],
                EffectDef::move_to_zone(
...
pub(in crate::card::sets) static ENTHUSIASTIC_MECHANAUT: CardRecord = CardRecord::new(
...
// Audit: unsupported — Needs variable loyalty costs and arbitrary graveyard-card copy tokens
...
// Audit: unsupported — The discard event also covers replacement destinations. An arbitrary
// triggering-card reference cannot be restricted to its graveyard before
// ExileGrantingControllerPlayThisTurn, so a replaced discard would incorrectly grant play
// permission.
pub(in crate::card::sets) static CONTAINMENT_CONSTRUCT: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static HIGH_SPEED_HOVERBIKE: CardRecord = CardRecord::new(
    "High-Speed Hoverbike",
    "7c619116-1eae-439d-9b1f-639643458a23",
    "Julian Kok Joon Wen",
    CardRules::new_artifact(mana_cost!("{2}"))
        .with_subtypes(&["Vehicle"])
        .with_abilities(&[
            abilities::flash(),
            abilities::flying(),
            abilities::enters_trigger_with_targets(
...
// Audit: unsupported — Needs a kind-agnostic counter transfer. "Put those counters on target
// creature" moves every kind the dying creature had, in the amounts it had, but AddCounters
// always names one kind and only removal has a kind-agnostic form. Narrowing it to +1/+1 would
// silently drop any other counter that reached this creature.
...
// NEO 250 — Mirror Box
// Audit: unsupported — Needs a static value evaluated relative to each affected creature so it
// can count other creatures sharing that creature's name.
                            ...
                            [
                                abilities::saga_chapter(
                                    ...
                                    EffectDef::CreateToken(CreateTokenDef::new(TokenDef::Literal(
                                        TokenCharacteristics::creature(&["Goblin", "Shaman"], &[ManaColor::Red], 2, 2)
                                        ...
                                        &const {
                                            crate::card::TokenCopyDef {
                                                ...
                                                exceptions: CopyExceptionsDef::NONE.with_abilities(
                                                    &const { [CopyAbilityDef::Ability(&abilities::haste())] },
                                                ),
                                            }
                                        },
                                    ))
...
pub(in crate::card::sets) static JIN_GITAXIAS_PROGRESS_TYRANT: CardRecord = CardRecord::new(
    "Jin-Gitaxias, Progress Tyrant",
    "01985566-275b-4bf0-8667-c81eb95ad70c",
    "Ai Nanahira",
    CardRules::new_creature(mana_cost!("{5}{U}{U}"), &["Phyrexian", "Praetor"], 5, 5)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            AbilityDef::triggered(
                "Whenever you cast an artifact, instant, or sorcery spell, \
                 copy that spell. You may choose new targets for the copy. \
                 This ability triggers only once each turn. (A copy of a \
            ...
            AbilityDef::triggered(
                "Whenever an opponent casts an artifact, instant, or sorcery \
                 spell, counter that spell. This ability triggers only once \
                 each turn.",
                TriggerEventDef::spell_cast(ObjectPredicateDef::All(&[
                    ObjectPredicateDef::AnyOf(&[
                        ObjectPredicateDef::HasType(CardType::Artifact),
...
pub(in crate::card::sets) static EIGANJO_SEAT_OF_THE_EMPIRE: CardRecord = CardRecord::new(
        ...
        .with_abilities(&[
            ...
            AbilityDef::activated_with_targets(
                "Channel — {2}{W}, Discard this card: It deals 4 damage to \
                 target attacking or blocking creature. This ability costs \
                 {1} less to activate for each legendary creature you \
                 control.",
...
pub(in crate::card::sets) static SOKENZAN_CRUCIBLE_OF_DEFIANCE: CardRecord = CardRecord::new(
        ...
        .with_abilities(&[
            ...
            AbilityDef::activated(
                "Channel — {3}{R}, Discard this card: Create two 1/1 \
                 colorless Spirit creature tokens. They gain haste until end \
                 of turn. This ability costs {1} less to activate for each \
                 legendary creature you control.",
                &[CostDef::Mana(mana_cost!("{3}{R}")), CostDef::DiscardSource],
                EffectDef::CreateToken(
                    crate::card::CreateTokenDef::new(crate::card::TokenDef::Literal(
                        crate::card::TokenCharacteristics::creature(&["Spirit"], &[], 1, 1),
                    ))
...
// NEO 436 — Farewell
pub(in crate::card::sets) static FAREWELL: CardRecord = CardRecord::new(
    "Farewell",
...
pub(in crate::card::sets) static THE_REALITY_CHIP: CardRecord = CardRecord::new(
    "The Reality Chip",
        ...
        .with_abilities(&[
            ...
            AbilityDef::static_ability(
                "As long as The Reality Chip is attached to a creature, you \
                 may play lands and cast spells from the top of your library.",
                EffectDef::IfCondition {
                    condition: &TriggerConditionDef::AttachedPermanentMatches {
                        object: ObjectPredicateDef::HasType(CardType::Creature),
                    },
            ...
            abilities::reconfigure(
                ...
                "Reconfigure {2}{U} ({2}{U}: Attach to target creature you \
                 control; or unattach from a creature. Reconfigure only as a \
                 sorcery. While attached, this isn't a creature.)",
            ),
        ]),
...
pub(in crate::card::sets) static TAKENUMA_ABANDONED_MIRE: CardRecord = CardRecord::new(
        ...
        .with_abilities(&[
            ...
            AbilityDef::activated(
                "Channel — {3}{B}, Discard this card: Mill three cards, then \
                 return a creature or planeswalker card from your graveyard \
                 to your hand. This ability costs {1} less to activate for \
                 each legendary creature you control.",
                &[CostDef::Mana(mana_cost!("{3}{B}")), CostDef::DiscardSource],
                EffectDef::Sequence(&[
                    EffectDef::Mill {
                        player: EffectRecipientDef::Controller,
                    ...
                    EffectDef::Choose(ChooseDef {
                        ...
                        minimum: 1,
                        maximum: 1,
                        binding: ObjectChoiceBindingDef::Objects(Binding!("takenuma_return")),
                        unchosen: None,
...
pub(in crate::card::sets) static CARDS: &[&CardRecord] = &[
    ...
    &MOON_CIRCUIT_HACKER,
    &MOONSNARE_PROTOTYPE,
    &TAMESHI_REALITY_ARCHITECT,
    &CLAWING_TORMENT,
    &OKIBA_RECKONER_RAID,
    &VIRUS_BEETLE,
    &CRACKLING_EMERGENCE,
    &EXPERIMENTAL_SYNTHESIZER,
    &GORO_GORO_DISCIPLE_OF_RYUSEI,
    &IRONHOOF_BOAR,
    &MARCH_OF_RECKLESS_JOY,
    &RABBIT_BATTERY,
    &TWINSHOT_SNIPER,
    &GREATER_TANUKI,
