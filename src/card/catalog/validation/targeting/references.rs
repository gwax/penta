...
pub(in crate::card::catalog) fn validate_ability_targets(
    ...
    validate_effect_references(effect, targets.len(), BindingScope::empty(&bindings))?;
...
pub(super) fn validate_ability_trigger_event(
    ...
    let bindings = BindingRegistry::default();
    validate_trigger_event_references(event, target_count, BindingScope::empty(&bindings))
}
    ...
    match condition {
        ...
        TriggerConditionDef::SourcePaidAlternativeCost(binding) => {
            if binding.label().is_some() && scope.cost_bindings.contains(&binding) {
                ...
                Err(
                    GrantedAbilityValidationError::InvalidAlternativeCostBinding {
                        binding,
                        reason: "no alternative cost in this card part declares that name",
                    },
                )
            }
        }
...
fn validate_object_set_target_references(
    ...
    match objects {
        ...
        ObjectSetDef::LegalTargets(target) => validate_target_index(target, target_count),
...
fn validate_applied_effect_target_references(
    ...
    match effect {
        ...
        AppliedEffectDef::Rule(AppliedRuleDef::AttackRestriction(restriction)) => {
            validate_object_predicate_references(restriction.attacker, target_count, scope)
        }
