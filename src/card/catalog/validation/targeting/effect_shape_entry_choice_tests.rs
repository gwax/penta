...
fn color_choices_require_durable_bindings_and_distinct_nonempty_sets() {
    const CHOICE: ReplacementEffectDef =
        ReplacementEffectDef::Choose(ReplacementChoiceDef::Colors(2));
    ...
    assert!(
        validate_value_shape(
            ValueDef::ColorIntersectionCount(&[crate::card::ColorSetDef::Binding(
                crate::ParentBinding
            ),]),
            &[]
        )
