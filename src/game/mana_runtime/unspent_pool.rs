...
impl Game {
    ...
    fn unspent_pool_after_mana_costs(
        ...
        let mut eligible = self.eligible_mana_pool_for_cost(
            player,
            &purpose,
            crate::card::costs::mana_cost(costs, None).unwrap_or_default(),
        );
        for cost in costs {
            if let CostDef::Mana(cost) = cost {
                ...
                let before = eligible;
                eligible = self.mana_payment_remainder(player, eligible, cost, 0, &purpose);
    ...
    pub(super) fn price_explicit_mana_production(
        &self,
        player: PlayerId,
        activation: &mut ManaAbilityActivation,
    ) {
        let Some(payment) = &self.explicit_mana_payment else {
            return;
        };
        let permanent = self
            .battlefield
            .iter()
            .find(|p| p.card.id == activation.source)
            .or_else(|| match self.retired_objects.get(&activation.source) {
        ...
        for index in &payment.units {
            pool.remove_color(available[*index].color, 1);
        }
        ...
        if let Some(value) = activation.effect.variable_amount {
            ...
            activation.effect.amount =
                self.mana_amount_for(activation.effect, player, activation.source);
        }
    }
