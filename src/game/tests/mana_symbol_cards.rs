...
fn phyrexian_life_is_reserved_from_channel() {
    ...
    exact_channel_game.players[PlayerId::One.index()].life = 3;
    ...
    let exact_channel_cast = life_paid_tamiyo_cast(&exact_channel_game, tamiyo_id)
        .expect("Channel can be activated before Tamiyo pays two life");
    exact_channel_game
        .apply(PlayerId::One, exact_channel_cast)
        .expect("the offered Channel and Phyrexian-life cast remains payable");
    assert_eq!(exact_channel_game.players[PlayerId::One.index()].life, 0);
}
...
fn phyrexian_life_is_reserved_from_life_cost_mana_sources() {
    ...
    mixed_game.players[PlayerId::One.index()].life = 4;
    ...
    assert_eq!(mixed_game.players[PlayerId::One.index()].life, 0);
    assert!(
        mixed_game
            .battlefield
            .iter()
