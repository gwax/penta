//! Talisman of Curiosity: colourless for nothing, or green or blue for a
//! life.
...
fn staged() -> (Game, GameObjectId) {
    ...
    let talisman = game
        .put_onto_battlefield(PlayerId::One, cards::TALISMAN_OF_CURIOSITY)
        .expect("cataloged");
...
fn it_makes_only_its_two_colors() {
    ...
    for color in [ManaColor::Red, ManaColor::Black, ManaColor::White] {
        assert!(
...
fn the_tap_pays_for_only_one_of_them() {
    ...
    assert!(mana_action(&game, talisman, ManaColor::Green).is_none());
...
fn the_life_it_costs_is_dealt_as_damage() {
    ...
    let green = mana_action(&game, talisman, ManaColor::Green).expect("it makes {G}");
