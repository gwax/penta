//! Talisman of Dominance: colourless for nothing, or blue or black for a
//! life.
...
fn staged() -> (Game, GameObjectId) {
    ...
    let talisman = game
        .put_onto_battlefield(PlayerId::One, cards::TALISMAN_OF_DOMINANCE)
        .expect("cataloged");
...
fn it_makes_only_its_two_colors() {
    ...
    for color in [ManaColor::Red, ManaColor::White, ManaColor::Green] {
        assert!(
...
fn the_tap_pays_for_only_one_of_them() {
    ...
    assert!(mana_action(&game, talisman, ManaColor::Black).is_none());
...
fn the_life_it_costs_is_dealt_as_damage() {
    ...
    let black = mana_action(&game, talisman, ManaColor::Black).expect("it makes {B}");
    game.apply(PlayerId::One, black).expect("it activates");
    game.check_state_based_actions();
