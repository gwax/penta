...
use crate::Format;
use crate::card::{CardCatalog, CardDefinition, ImplementationStatus};

mod requirements;
pub(crate) use requirements::validate_companion_requirement;
pub use requirements::{DeckRequirementViolation, validate_deck_requirement};
...
impl Deck {
    ...
    /// Validate a candidate against the complete starting deck, including commanders.
    /// Another copy in the main deck is allowed if the requirement permits it.
    /// # Errors
    /// Reports a missing sideboard card, unknown identity, or failed requirement.
    pub fn validate_companion(
        ...
        if !self.sideboard.contains(&companion) {
            return Err(DeckRequirementViolation::NotInSideboard(companion));
        }
        validate_companion_requirement(
            companion,
            catalog,
            self.main.iter().chain(&self.commanders).copied(),
            format,
        )
    }
