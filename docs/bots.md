...
The optional `reconstruction.checkpoint.v21` capability advertises a hidden-safe
...
values and makes a pending card-name decision carry only the name binding,
rather than one zone whose matching cards were implicitly bound by the choice.
Format 13 makes that decision a bindable sequence step and stores the remaining
ordered effect procedure to resume after the answer.
...
The optional `castAlternativeCostBinding` field on stack objects (including
detached objects) and permanents stores the chosen alternative's authored name,
such as `"evoke"`. Unlabeled and external alternatives such as Omniscience
have no binding. This choice survives spell copies and lets Evoke distinguish
its cost from another alternative of the same kind, independent of ability
order. The wire stores the name, not its internal interned ID. When the field
is absent, a stack object's retained cast signature supplies the binding if
available; a permanent has no such fallback.
...
Action alternatives use the ordinary choice shape: first choose a complete
action, then select its exact object group. Resolving payments may decline;
optional instructions use an ordinary May decision. Forage uses three graveyard
cards or one controlled Food. The `actionChoice` checkpoint continuation retains
...
Checkpoint format 20 introduced storage for this designation, whether its original object
...
current reconstruction consumers must require
`reconstruction.checkpoint.v21` and regenerate older checkpoints. Ordinary bot
...
and pending special-action payments as `paySpecialAction`. Reconstruction
validates the complete payment against its source declaration. Consumers using
reconstruction must require `reconstruction.checkpoint.v14` and the matching
simulation fingerprint. Ordinary indexed actions and replay version 2 are
unchanged.

### Migrating checkpoint format 12 to 13

Protocol 30 and replay format 2 remain in place. Checkpoint format 13 makes a
resolving card-name choice an ordinary bindable sequence step. A pending
`cardNameChoice` now stores a `resume` procedure containing the remaining
ordered effects instead of a singular `continuation`, so reconstruction
preserves the same sequence structure used by the declarative card definition.
Reconstruction consumers should require `reconstruction.checkpoint.v13` and
regenerate format-12 checkpoints with the current engine.
...
the separate singular, object-set, and named-output binding stores with one
typed `bindings` map. Each labeled entry is either a zero-or-one object or an
ordered object group. Values passed only to an effect's direct continuation
are serialized separately as `parentObject` or `parentObjects` and need no
durable label.

Format 10 cannot distinguish those lexical parent values from durable labeled
outputs after this model change. Reconstruction consumers should require
`reconstruction.checkpoint.v11`, keep checking the exact simulation
fingerprint, and regenerate format-10 checkpoints with the current engine.
...
The current optional capability is `reconstruction.checkpoint.v21`. An ordinary
